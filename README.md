# Victor Caetano da Conceição
`Desenvolver FullStack`
Ola sou Victor Caetano da Conceição tenho 21 anos moro no Espirito Santo faço falculdade de analise e desenvolvimento de sistemas (Unisales) 


- Foco - desenvolver tanto front-end e back-end visando criar sistemas completos
- Grow - tenho buscado aprender React (next e expo), flutter e jwt(web token) em varias linguagens(Java OK), typescript

## Faculdade
Onde aprendi gestão de projetos com metodologia agil (kanban e scrum), estrutura de dados, SQL Banco de Dados(postgresSQLm Mysql,sqllite(para coisas pequenas))e Java eo framework Spring Boot 

A cada periodo tive grande evolução de acordo com os projetos para o Projeto integrador 
- Mit app inventor (1ºperiodo e 2ºperiodo)  
- Api crud (javascript) (3ºperiodo)
- React Next com Spring Boot, Web, Jpa e Token(4ºperiodo)

<!---
victorcaetano2025/victorcaetano2025 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
